...
let ashibka = j.door = 1;
console.log(ashibka);

//Task 3
class ArraySum extends Array {
    sum() {
        return this.filter(value => typeof value === "number").reduce((gumar,value) => gumar + value,0);
    }
};
let arraysum = new ArraySum(1,2,3,4,5,"poxos",5,"Petros",true,10);
...
String.prototype.upper = function() {
    ...
    strArray = strArray.map((value) => {
        return value.charAt(0).toUpperCase() + value.substring(1);
    })
    return strArray.join(" ");
};

let str = "i am a child";
console.log(str.upper());

console.log("you are a boy".upper());
